...
#plot(SALIX.mds2)
```
# Repeating process with other species (Need to change code)
```{r}
SALIX <- load_data("SALIX")
d <- dissim(SALIX)
process <- best_clustering(SALIX, d, "SALIX", 20)
...
SALIX.mds2.df<-data.frame(SALIX.mds2$species)
SALIX.mds2.df
write_csv(SALIX.mds2.df, "SALIX.mds2.df.csv") #doesn't contain PID
...
# Repeating process with other species
```{r}
SALIX <- load_data("SALIX")
d <- dissim(SALIX)
process <- best_clustering(SALIX, d, "SALIX", 20)
