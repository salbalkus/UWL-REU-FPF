...
```
```{r}
SALIX.mds2alt<-metaMDS(d,distance="manhattan",k=2, trymax=100,maxit=500,autotransform = FALSE) 
plot(SALIX.mds2alt)
...
```{r}
proc.time() - ptm
SALIX.mds3<-metaMDS(d,distance="manhattan",k=3, trymax=100)
SALIX.mds4<-metaMDS(d,distance="manhattan",k=4, trymax=100)
SALIX.mds5<-metaMDS(d,distance="manhattan",k=5, trymax=100)
proc.time() - ptm
```
```{r}
library(vegan)
...
library(goeveg)
#Community data, a matrix-like object with samples in rows and species in columns.
dimcheckMDS(as.matrix(d), distance = "manhattan", k = 6, trymax = 5000,autotransform = TRUE)
```
